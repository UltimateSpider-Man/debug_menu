...
int& scan_pos = var<int>(0x009684F8);
char*& line = var<char*>(0x009684FC);
char& token = var<char>(0x009683F8);
char& stored_token = var<char>(0x00968500);
char& stored_type = var<char>(0x00968501);
char& stored_num = var<char>(0x00968502);

    inline auto& filename = var<char[255]>(0x009682F8);



void new_line(char* lpBuffer)
{
    line = lpBuffer;
    scan_pos = 0;
}

signed int build_token(char* a1, char* a2)
{
    char* v2 = a1;
    char v3 = *a1;
    signed int result = 0;
...
int get_token(char** a1, int* a2, int* a3)
{
    int result;
    char* v4;
    int v5;
    ...
    if (stored_token) {
        ...
        *a3 = stored_num;
        ...
        result = build_token(&line[scan_pos],
            &token);
        scan_pos = result + v5;
        if (result) {
            *a1 = &token;
...
void parse(const char* ini_filename, os_developer_options* a2)
{
    if constexpr (1) {
        ...
        if (file.opened) {
            ...
            int token_type;
            if (get_token(&Str, &token_type, &v11)) {
                ...
                while (1) {
                    if (token_type == 1) {
                        ...
                        if (get_token(&Str, &token_type, &v11) == 3) {
                            get_token(&Str, &token_type, &v11);
            ...
            printf("Error.  Could not open ini file %s\n", filename);
            // assert(0);
        }
        ...
        // sub_59B6F0(&file);
                ...
                void(__fastcall * func)(void*, void*, const char*, os_developer_options*) = bit_cast<decltype(func)>(0x005CA120);

        func(0, nullptr, ini_filename, a2);
    }
}
