...
void create_district_variants_menu(debug_menu* parent)
{

    auto* district_variants_menu = create_menu("District variants", debug_menu::sort_mode_t::ascending);
    auto* v2 = create_menu_entry(district_variants_menu);
    debug_menu_entry v1;
    debug_menu_entry* block = v1.alloc_block(district_variants_menu, 4);
    block[0] = debug_menu_entry{ district_variants_menu };
    v2->set_game_flags_handler(populate_district_variants_menu);
    parent->add_entry(v2);
}
...
inline void ngl_entry_handler(debug_menu_entry* entry)
{
    auto v3 = entry->get_bval();
    entry->set_bval(v3, !v3);
}

inline void create_ngl_menu(debug_menu* parent)
{
    assert(parent != nullptr);

    auto* v22 = create_menu("NGL Debug");
    debug_menu_entry v20;
    v20.entry_type = UNDEFINED;
    auto* v133 = v22;
    auto* v23 = create_menu_entry(v133);

    parent->add_entry(v23);
    if (nglGetDebugFlagPtr("ShowPerfInfo") != nullptr) {
        auto* v21 = create_menu_entry(mString { "ShowPerfInfo" });

        auto v0 = nglGetDebugFlag("ShowPerfInfo");
        v21->set_ival(v0);
        v21->set_min_value(0.0);
        v21->set_max_value(2.0);
        v21->set_game_flags_handler(ngl_handler);
        v21->set_render_cb(ngl_render_callback);
        v133->add_entry(v21);
    }
    if (nglGetDebugFlagPtr("ShowPerfBar") != nullptr) {
        auto* v24 = create_menu_entry(mString { "ShowPerfBar" });

        auto v1 = nglGetDebugFlag("ShowPerfBar");
        v24->set_ival(v1);
        v24->set_min_value(0.0);
        v24->set_max_value(1.0);
...
void game_flags_handler(debug_menu_entry* a1)
{
    ...
    switch (a1->get_id()) {
    case ENABLE_PHYSICS: // Physics Enabled
    {
        auto v1 = a1->get_bval();
...
void replayHandler(debug_menu_entry* entry)
{
    if (!entry->get_id()) {
        debug_menu v1;
        rtdt_replay_mgr* instance = rtdt_replay_mgr::get_instance();
        instance = rtdt_replay_mgr::get_instance();
        if (rtdt_replay_mgr::function_instance(instance, 0)) {
            instance = rtdt_replay_mgr::get_instance();
            rtdt_replay_mgr::function_instance(instance, 1);
            g_game_ptr()->enable_physics(false);
            debug_menu::physics_state_on_exit = 0;
        }
        close_debug();
    }
}
...
void apply_variant_handler(debug_menu_entry* entry) {
    ...
    auto data = static_cast<conglomerate*>(entry->get_data());
    if (!data) {
        // Handle null data
        return;
    }

    auto variantName = entry->get_name();
    string_hash hash{ variantName.c_str() };

    auto variant_ifc = data->m_variant_ifc;
    if (!variant_ifc) {
        // Handle null variant interface
        return;
    }

    variant_ifc->apply_variant(hash);
}
*/


void populate_variants_menu(debug_menu_entry* a1)
{
    auto* entity_variants_menu = create_menu("Entity Variants", debug_menu::sort_mode_t::ascending);
    a1->set_submenu(entity_variants_menu);

    auto* entity_variants_menu0 = create_menu("PED_0", debug_menu::sort_mode_t::ascending);
    auto* v0 = create_menu_entry(entity_variants_menu0);
    //   v0->set_game_flags_handler(apply_variant_handler);
    entity_variants_menu->add_entry(v0);

    auto* entry = create_menu_entry(mString{ "__default" });
    entity_variants_menu0->add_entry(entry);

    auto* entry1 = create_menu_entry(mString{ "pf_ctpnts" });
    entity_variants_menu0->add_entry(entry1);

    auto* entry2 = create_menu_entry(mString{ "pf_ctskrt" });
    entity_variants_menu0->add_entry(entry2);

    auto* entry3 = create_menu_entry(mString{ "pf_shtpnts" });
    ...
    auto* entity_variants_menu1 = create_menu("PED_1", debug_menu::sort_mode_t::ascending);
    auto* v3 = create_menu_entry(entity_variants_menu1);
    //  v3->set_game_flags_handler(apply_variant_handler);
    ...
    auto* entity_variants_menu2 = create_menu("PED_2", debug_menu::sort_mode_t::undefined);
    auto* v4 = create_menu_entry(entity_variants_menu2);
    // v4->set_game_flags_handler(apply_variant_handler);
    ...
    auto* entity_variants_menu3 = create_menu("PED_3", debug_menu::sort_mode_t::undefined);
    auto* v5 = create_menu_entry(entity_variants_menu3);
    //   v5->set_game_flags_handler(apply_variant_handler);
    ...
    auto* entity_variants_menu4 = create_menu("PED_4", debug_menu::sort_mode_t::undefined);
    auto* v6 = create_menu_entry(entity_variants_menu4);
    // v6->set_game_flags_handler(apply_variant_handler);
    ...
    auto* entity_variants_menu5 = create_menu("PED_5", debug_menu::sort_mode_t::undefined);
    auto* v7 = create_menu_entry(entity_variants_menu5);
    // v7->set_game_flags_handler(apply_variant_handler);
    ...
    auto* entity_variants_menu6 = create_menu("PED_6", debug_menu::sort_mode_t::undefined);
    auto* v8 = create_menu_entry(entity_variants_menu6);
    //  v8->set_game_flags_handler(apply_variant_handler);
    ...
    auto* entity_variants_menu7 = create_menu("PED_7", debug_menu::sort_mode_t::undefined);
    auto* v9 = create_menu_entry(entity_variants_menu7);
    //  v9->set_game_flags_handler(apply_variant_handler);
    ...
    auto* entity_variants_menu8 = create_menu("PED_8", debug_menu::sort_mode_t::undefined);
    auto* v10 = create_menu_entry(entity_variants_menu8);
    //  v10->set_game_flags_handler(apply_variant_handler);
    ...
    auto* entity_variants_menu9 = create_menu("PED_9", debug_menu::sort_mode_t::undefined);
    auto* v11 = create_menu_entry(entity_variants_menu9);
    //   v11->set_game_flags_handler(apply_variant_handler);
...
void populate_entity_variants_menu(debug_menu_entry* entry) {
    ...
    auto entities = *entity::found_entities();

    for (auto& entity : entities) {
        auto entityName = entity->get_id().to_string();
        auto entityEntry = create_menu_entry(entityName);
        entityEntry->set_game_flags_handler(populate_variants_menu); // Uncommented this line
...
void create_entity_variants_menu(debug_menu* parent) {
    auto entry = std::unique_ptr<debug_menu_entry>(create_menu_entry("Entity Variants"));
    entry->set_submenu(nullptr);
    entry->set_game_flags_handler(populate_entity_variants_menu);
    parent->add_entry(entry.get());
    entry.release(); // Assuming parent takes ownership
}
