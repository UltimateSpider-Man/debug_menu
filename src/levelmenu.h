...
inline enum class hero_status_e {
    UNDEFINED = 0,
    REMOVE_PLAYER = 1,
    ADD_PLAYER = 2,
...
inline int hero_selected;
inline int frames_to_skip = 2;
...
inline level_descriptor_t *get_level_descriptors(int *arg0)
{
...
inline level_descriptor_t* get_hero_list_updator(int* hero)
{
    auto* game_partition = resource_manager::get_partition_pointer(0);
    assert(game_partition != nullptr);
    ...
    if (hero != nullptr)
    {
        *hero = a2 / sizeof(level_descriptor_t);
    }
...
inline void hero_entry_callback(debug_menu_entry *entry);

inline void hero_toggle_handler(debug_menu_entry *entry);
...
inline void create_level_select_menu(debug_menu *level_select_menu)
{
    ...
    int hero;
    auto* hero_update_list = get_hero_list_updator(&hero);
    printf("num_descriptors = %d\n", hero);
    for (auto i = 0; i < hero; ++i)
    {
        auto v6 = 25;
        auto* v1 = hero_update_list[i].field_0.to_string();
        string_hash v5{ v1 };
        auto v11 = resource_key{ v5, v6 };
        auto v17 = resource_manager::get_pack_file_stats(v11, nullptr, nullptr, nullptr);
        if (v17)
        {
            mString v23{ hero_update_list[i].field_60.to_string() };
            debug_menu_entry v37{ v23.c_str() };
...
inline void hero_entry_callback(debug_menu_entry* entry)
{
    ...
    case hero_status_e::REMOVE_PLAYER:
    {
        ...
        frames_to_skip = 2;
        g_game_ptr()->enable_marky_cam(true, true, 0.0, -1000.0);
        break;
    }
    case hero_status_e::ADD_PLAYER: {
        ...
        if (v1 <= 0) {
            ...
            // Create an mString from the hero name

            g_game_ptr()->enable_marky_cam(false, true, 0.0, -1000.0);
            frames_to_skip = 2;
    ...
    case hero_status_e::CHANGE_HEALTH_TYPE: {
        ...
        if (v3 <= 0) {
            auto v17 = 0;
            auto* v5 = (camera*)g_world_ptr()->get_hero_ptr(0);
