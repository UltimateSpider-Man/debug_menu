...
public:
    ...
    static inline void (*setup_inputs_p)(game*) = game__setup_inputs;
          // modernised replacement for sub_CE94F0
        void clear() noexcept {

        }             // wraps old sub_6A5E20

    std::vector<std::unique_ptr<FEText>> districtTexts_; // replaces raw ** array
    std::vector<std::unique_ptr<FEText>> stripTexts_;    // idem
    bool initialised_ = false;

    float dword_14F2C4C = 0.0f;
};
